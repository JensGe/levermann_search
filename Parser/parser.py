...
def write_index_contents_from_html_to_db():
    ...
    for file in file_list:
        ...
        db.write_stock_list_to_db(stock_list, index_uri) # TODO DB _mysql_exceptions.OperationalError: (1040, 'Too many connections')
...
def write_stock_overview_data_to_db():
    ...
    for file in file_list:
        ...
        if stock_overview_soup is None:
            logger.warning("Write Stock Overview Data to DB: Stock Overview Soup is None for %s" % str(file))
            continue

        market_cap = parse.get_market_cap(stock_overview_soup)
        if market_cap is None:
            logger.warning("Write Stock Overview Data to DB: Market Cap is None for %s" % str(file))
            market_cap = -1.00
...
def write_stock_histories_from_html_to_db():
    ...
    for file in file_list:
        ...
        if stock_history_soup is None:
            logger.warning("Write Stock History Data to DB: Stock History Soup is None for %s" % str(file))
            continue
...
def write_stock_balance_data_to_db():
    ...
    for file in file_list:
        ...
        if stock_balance_soup is None:
            logger.warning("Write Stock Balance Data to DB: Soup is None for %s" % str(file))
            continue

        if not parse.is_data_available(stock_balance_soup):
            logger.warning("Write Stock Balance Data to DB: No Data Available for %s" % str(file))
            continue
        ...
        try:
            ...
            logger.exception("Write Stock Balance Data to DB: Exception for stock: %s" % stock_uri)
            continue
...
def write_stock_estimates_data_to_db():
    ...
    for file in file_list:
        ...
        if stock_estimate_soup is None:
            logger.warning("Write Stock Estimate Data to DB: Soup is None for %s" % str(file))
            continue
        ...
        try:
            ...
            logger.exception("Write Stock Estimate Data to DB: Exception for stock: %s" % stock_uri)
...
def write_stock_targets_data_to_db():
    ...
    for file in file_list:
        ...
        if stock_targets_soup is None:
            logger.warning("Write Stock Targets Data to DB: Soup is None for %s" % str(file))
            continue
        ...
        try:
            ...
            logger.exception("Write Stock Targets Data to DB: Exception for stock: %s" % stock_uri)
...
def write_stock_last_quarterly_figures_date_to_db():
    ...
    for file in file_list:
        ...
        if stock_targets_soup is None:
            logger.warning("Write Stock Quaterlyfigures Data to DB: Soup is None for %s" % str(file))
            continue
        try:
            ...
            logger.exception("Write Stock Quaterly Data to DB: Exception for stock: %s" % stock_uri)
