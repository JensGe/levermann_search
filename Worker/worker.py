...
def scrap_single_page(driver, scrap_url, save_path):
    ...
    if os.path.isfile(file_name):
        logger.info(
            "Single Scrap Stock %s: Skip existing File for stock: %s"
            % (save_path.split("/")[-2], stock_uri)
        )
        return
    else:
...
def generate_scrap_urls(stock_uri):
    return {
        "Overview Url": cst.URL_STOCK_OVERVIEW + stock_uri,
...
def start_complete_workflow():
    stock_uri_list = db.get_list(
        table="Aktien_Scraping_Queue",
        columns="AktienURI",
    ...
    for stock in stock_uri_list:
        ...
        scrap_single_page(
            driver=driver,
            scrap_url=scrap_urls["Overview Url"],
            save_path=cst.PATH_STOCK_OVERVIEW,
        )
        scrap_single_page(
            driver=driver,
            scrap_url=scrap_urls["Estimates Url"],
            save_path=cst.PATH_STOCK_ESTIMATES,
        )
        scrap_single_page(
            driver=driver,
            scrap_url=scrap_urls["Dates Url"],
            save_path=cst.PATH_STOCK_DATES,
        )

        bygone_dates, future_dates = parse_dates(
            scrap.get_soup_code_from_file(cst.PATH_STOCK_DATES + stock)
        )

        # ToDo
        db.write_date_list_to_db(future_dates)
        db.write_date_list_to_db(bygone_dates)
